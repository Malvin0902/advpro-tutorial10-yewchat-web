...
// Message enum with reaction support
pub enum Msg {
    HandleMsg(String),
    SubmitMessage,
    ChangeTheme(Theme),
...
impl Theme {
    fn get_css_classes(&self) -> &str {
        match self {
            ...
            Theme::Ocean => "bg-blue-900 text-white",
            Theme::Forest => "bg-green-900 text-white",
        }
    }
}

// Main chat component with reactions
pub struct Chat {
            ...
            <div class={format!("flex w-screen {}", theme_classes)}>
                ...
                <div class="flex-none w-56 h-screen bg-opacity-90 bg-gray-100">
                    <div class="p-3 flex justify-between items-center">
                        <div class="text-xl text-black">{"Users"}</div>
                        <select onchange={theme_callback} class="px-2 py-1 rounded bg-white text-black">
                                ...
                                html! {
                                    <div class="flex m-3 bg-white rounded-lg p-2 shadow-sm hover:shadow-md transition-shadow duration-200">
                                        ...
                                        <div class="flex-grow p-3">
                                            <div class="flex text-xs justify-between font-bold text-black">
                                        ...
                                        <div class={format!("flex flex-col ml-{}", if is_new_user { "0" } else { "10" })}>
                                            <div class="max-w-3/4 bg-gray-100 p-3 rounded-lg shadow-sm">
                                                if m.message.ends_with(".gif") {
                                                    <img class="max-h-64 rounded" src={m.message.clone()} alt="gif"/>
                                                } else {
                                                    <div class="text-sm whitespace-pre-wrap break-words text-black">
                                                        {m.message.clone()}
                                                    </div>
                                                }
                                            </div>
                                                        ...
                                                        reactions.iter().map(|(emoji, count)| {
                                                            ...
                                                            html! {
                                                                <button 
                                                                    ...
                                                                    class="bg-gray-200 rounded-full px-2 py-1 text-xs mr-1 mb-1 text-black hover:bg-gray-300"
                                                                >
                                                                    {format!("{} {}", emoji, count)}
                                                                </button>
                                                            }
                    ...
                    <div class="w-full flex flex-col px-3 py-2 relative">
                        ...
                        <div class="flex items-center">
                            <input 
                                ...
                                class="block w-full py-2 pl-4 mx-3 bg-gray-100 rounded-full outline-none focus:ring-2 focus:ring-blue-600 text-black" 
                                name="message" 
                                required=true 
                            />
                            <button 
